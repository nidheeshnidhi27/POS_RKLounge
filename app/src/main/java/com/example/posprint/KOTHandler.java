...
//    hide price from kot print
    /*private String formatItemLine(String qty, String name, double total) {

        String price = "£" + String.format(Locale.US, "%.2f", total);

        int lineWidth = 30; // your 58mm item width

        String firstLineLeft = qty + "x " + name;

        // If full text fits in one line → print directly
        if (firstLineLeft.length() <= lineWidth - price.length() - 1) {
            int spaces = lineWidth - firstLineLeft.length() - price.length();
            if (spaces < 1) spaces = 1;

            return firstLineLeft + " ".repeat(spaces) + price;
        }

        // ⚠ WRAP item name properly
        // 1. Split into first line (max allowed)
        int maxFirstLineChars = lineWidth - price.length() - 1;
        String firstLineText = firstLineLeft.substring(0, maxFirstLineChars);

        // 2. Remaining text → next line
        String remaining = firstLineLeft.substring(maxFirstLineChars).trim();

        // 3. Build the multi-line output
        StringBuilder out = new StringBuilder();

        // First line with price
        int spaces = lineWidth - firstLineText.length() - price.length();
        if (spaces < 1) spaces = 1;
        out.append(firstLineText).append(" ".repeat(spaces)).append(price);
